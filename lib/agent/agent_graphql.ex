...
if Application.compile_env(:bonfire_api_graphql, :modularity) != :disabled do
  defmodule ValueFlows.Agent.GraphQL do
    ...
    def people(page_opts, info) do
      people_pages =
        if Bonfire.Common.Extend.module_enabled?(CommonsPub.Web.GraphQL.UsersResolver) do
          with {:ok, users_pages} <-
                 CommonsPub.Web.GraphQL.UsersResolver.users(page_opts, info) do
            people =
              Enum.map(
                users_pages.edges,
                &(&1
          ...
          people =
            ValueFlows.Agent.People.people(nil)
            |> Enum.map(
              &(&1
                |> ValueFlows.Agent.Agents.character_to_agent())
            )
    ...
    def person(%{id: id}, info) do
      {:ok,
       ValueFlows.Agent.People.person(
         id,
    ...
    def organizations(page_opts, info) do
      orgz_pages =
        if Bonfire.Common.Extend.module_enabled?(Organisation.GraphQL.Resolver) do
          with {:ok, pages} <-
                 Organisation.GraphQL.Resolver.organisations(page_opts, info) do
            orgz =
              Enum.map(
                pages.edges,
                &(&1
    ...
    def my_agent(_, info) do
      with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info) do
        {:ok, ValueFlows.Agent.Agents.character_to_agent(user)}
      end
    end
