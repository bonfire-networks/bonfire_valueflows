...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.EconomicEvent.GraphQL do
    ...
    def simulate(_, _) do
      {:ok,
       Bonfire.Common.Simulation.some(
         1..5,
         &ValueFlows.Simulate.economic_event/0
       )}
    end
    ...
    def fetch_resource_inventoried_as_edge(
          %{resource_inventoried_as: {:error, _} = error},
          _,
          _
        ),
        do: error
    ...
    def fetch_to_resource_inventoried_as_edge(
          %{to_resource_inventoried_as: {:error, _} = error},
          _,
          _
        ),
        do: error
    ...
    def fetch_to_resource_inventoried_as_edge(
          %{to_resource_inventoried_as_id: id} = thing,
          _,
          _
        )
        when not is_nil(id) do
    ...
    def delete_event(%{id: id}, info) do
      repo().transact_with(fn ->
        with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
             {:ok, event} <- event(%{id: id}, info),
             :ok <- ValueFlows.Util.can?(user, :delete, event),
