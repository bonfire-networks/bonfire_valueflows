defmodule ValueFlows.EconomicEvent.Trace do
  ...
  def trace(id, recurse_limit, recurse_counter) when is_binary(id) do
    with {:ok, obj} <-
           Bonfire.Common.Pointers.get(id, skip_boundary_check: true) do
  ...
  defp maybe_recurse(objects, recurse_limit, recurse_counter)
       when is_nil(recurse_limit) or recurse_limit > @max_recurse_limit,
       do: maybe_recurse(objects, Util.default_recurse_limit(), recurse_counter)

  defp maybe_recurse(objects, recurse_limit, recurse_counter)
       when is_nil(recurse_limit) or recurse_counter + 1 < recurse_limit do
  ...
  defp recurse(objects, recurse_limit, recurse_counter)
  ...
  def resource(resource_or_id, recurse_limit, recurse_counter) do
    with {:ok, events} <-
           EconomicEvents.many([:default, trace_resource: ulid(resource_or_id)]) do
      {:ok, maybe_recurse(events, recurse_limit, recurse_counter)}
  ...
  def event(event, recurse_limit, recurse_counter) do
    with {:ok, resources} <- trace_resource_input(event),
         ...
         {:ok, process} <- trace_process_output(event) do
      {
        :ok,
        resources
        |> Enums.maybe_append(resource_inventoried_as)
  ...
  defp trace_resource_input(%{input_of_id: input_of_id})
       when not is_nil(input_of_id) do
  ...
  defp trace_resource_inventoried_as(
       ...
       when action_id in ["transfer", "move"] and
              not is_nil(resource_inventoried_as_id) do
  ...
  defp trace_process_output(%{output_of_id: output_of_id})
       when not is_nil(output_of_id) do
