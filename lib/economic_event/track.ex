defmodule ValueFlows.EconomicEvent.Track do
  ...
  defp maybe_recurse(objects, recurse_limit, recurse_counter)
       when recurse_counter + 1 < recurse_limit do
    debug("Track: recurse level #{recurse_counter} of #{recurse_limit}")
  ...
  def resource(resource_or_id, recurse_limit, recurse_counter) do
    with {:ok, events} <-
           EconomicEvents.many([:default, track_resource: ulid(resource_or_id)]) do
  ...
  defp track_to_resource_output(
       ...
       when action_id in ["transfer", "move"] and
              not is_nil(to_resource_inventoried_as_id) do
  ...
  defp track_resource_output(%{output_of_id: output_of_id})
  ...
  defp track_process_input(%{input_of_id: input_of_id})
