...
if Application.compile_env(:bonfire_api_graphql, :modularity) != :disabled do
  defmodule ValueFlows.EconomicResource.GraphQL do
    ...
    def simulate(_, _) do
      {:ok,
       Bonfire.Common.Simulation.some(
         1..5,
         &ValueFlows.Simulate.economic_resource/0
       )}
    end
    ...
    defp resources_filter(
           %{tracking_identifier: tracking_identifier} = page_opts,
           filters_acc
         ) do
      resources_filter_next(
        :tracking_identifier,
    ...
    defp resources_filter(
           %{current_location: current_location_id} = page_opts,
           filters_acc
         ) do
      resources_filter_next(
        :current_location,
    ...
    def spec_conforms_to_resources(
          %{conforms_to: spec_id},
          %{} = _page_opts,
          _info
        ) do
    ...
    def spec_conforms_to_resources_edge(
          %{conforms_to: spec_id},
          %{} = page_opts,
          info
        ) do
    ...
    def fetch_conforms_to_edge(%{conforms_to_id: id} = thing, _, _)
        when is_binary(id) do
    ...
    def create_resource(%{new_inventoried_resource: resource_attrs}, info) do
      with {:ok, resource} <-
             create_resource(%{economic_resource: resource_attrs}, info) do
