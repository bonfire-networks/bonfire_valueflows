...
defmodule ValueFlows.Knowledge.ProcessSpecification.ProcessSpecifications do
  ...
  * Various parts of the codebase that need to query for this (inc. tests)
  """
  def one(filters),
    do: repo().single(Queries.query(ProcessSpecification, filters))
  ...
  * Various parts of the codebase that need to query for this (inc. tests)
  """
  def many(filters \\ []),
    do: {:ok, repo().many(Queries.query(ProcessSpecification, filters))}
  ...
  * GraphQL resolver bulk resolution
  ...
  def update(%ProcessSpecification{} = process_spec, attrs) do
    repo().transact_with(fn ->
      ...
      with {:ok, process_spec} <-
             repo().update(ProcessSpecification.update_changeset(process_spec, attrs)),
           {:ok, process_spec} <-
             ValueFlows.Util.try_tag_thing(nil, process_spec, attrs),
  ...
  def soft_delete(%ProcessSpecification{} = process_spec) do
    repo().transact_with(fn ->
      with {:ok, process_spec} <-
             Bonfire.Common.Repo.Delete.soft_delete(process_spec),
