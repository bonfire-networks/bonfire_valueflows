...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.Knowledge.ProcessSpecification.GraphQL do
    ...
    def simulate(_, _) do
      {:ok,
       Bonfire.Common.Simulation.some(
         1..5,
         &ValueFlows.Simulate.process_specification/0
       )}
    end
    ...
    defp process_specs_filter_next(
           param_remove,
           filter_add,
           page_opts,
           filters_acc
    ...
    def create_process_spec(%{process_specification: process_spec_attrs}, info) do
      repo().transact_with(fn ->
        with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
             ...
             {:ok, process_spec} <-
               ProcessSpecifications.create(user, process_spec_attrs) do
    ...
    def delete_process_spec(%{id: id}, info) do
      repo().transact_with(fn ->
        with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
             {:ok, process_spec} <- process_spec(%{id: id}, info),
             :ok <- ValueFlows.Util.can?(user, :delete, process_spec),
