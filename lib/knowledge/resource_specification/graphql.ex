...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.Knowledge.ResourceSpecification.GraphQL do
    ...
    * GraphQL resolver bulk resolution
    ...
    def simulate(_, _) do
      {:ok,
       Bonfire.Common.Simulation.some(
         1..5,
         &ValueFlows.Simulate.resource_specification/0
       )}
    end
    ...
    def fetch_default_unit_of_effort_edge(
          %{default_unit_of_effort_id: id} = thing,
          _,
          _
        )
    ...
    def fetch_conforming_resources_edge(%{conforms_to: id}, page_opts, info)
        when not is_nil(id) do
      ValueFlows.EconomicResource.GraphQL.spec_conforms_to_resources(
        %{conforms_to: id},
        page_opts,
        info
      )
    ...
    def create_resource_spec(
          %{resource_specification: resource_spec_attrs},
          info
        ) do
      repo().transact_with(fn ->
        with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
             ...
             {:ok, resource_spec} <-
               ResourceSpecifications.create(user, resource_spec_attrs) do
    ...
    def update_resource_spec(
          %{resource_specification: %{id: id} = changes},
          info
        ) do
      with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
    ...
    def delete_resource_spec(%{id: id}, info) do
      repo().transact_with(fn ->
        with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
             {:ok, resource_spec} <- resource_spec(%{id: id}, info),
             :ok <- ValueFlows.Util.can?(user, :delete, resource_spec),
