...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.ValueCalculation.GraphQL do
    ...
    def value_unit_edge(%{value_unit_id: id} = thing, _, _)
    ...
    def resource_conforms_to_edge(%{resource_conforms_to_id: id} = thing, _, _)
        when is_binary(id) do
    ...
    def value_resource_conforms_to_edge(
          %{value_resource_conforms_to_id: id} = thing,
          _,
          _
        )
        when is_binary(id) do
    ...
    def update_value_calculation(%{value_calculation: %{id: id} = attrs}, info) do
      with :ok <- GraphQL.is_authenticated(info),
           ...
           {:ok, value_calculation} <-
             ValueCalculations.update(value_calculation, attrs) do
        {:ok, %{value_calculation: value_calculation}}
      end
    end
