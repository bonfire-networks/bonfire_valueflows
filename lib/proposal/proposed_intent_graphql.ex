  ...
  defmodule ValueFlows.Proposal.ProposedIntentGraphQL do
    ...
    def intent_in_proposal_edge(%{id: proposed_intent_id}, _, info) do
      with {:ok, proposed_intent} <-
             ValueFlows.Proposal.ProposedIntents.one([
               :default,
               id: proposed_intent_id
             ]) do
        ValueFlows.Planning.Intent.GraphQL.intent(
          %{id: proposed_intent.publishes_id},
          info
        )
      end
    end
    ...
    def proposal_in_intent_edge(%{id: proposed_intent_id}, _, info) do
      with {:ok, proposed_intent} <-
        ...
        ValueFlows.Proposal.GraphQL.proposal(
          %{id: proposed_intent.published_in_id},
          info
        )
      end
    end
    ...
    def publishes_edge(%{id: proposal_id}, _, _info) do
      ValueFlows.Proposal.ProposedIntents.many([
        :default,
        published_in_id: proposal_id
      ])
    end
    ...
    def propose_intent(
          %{
            published_in: published_in_proposal_id,
            publishes: publishes_intent_id
          } = params,
          info
        ) do
      with :ok <- GraphQL.is_authenticated(info),
           {:ok, published_in} <-
             ValueFlows.Proposal.GraphQL.proposal(
               %{id: published_in_proposal_id},
               info
             ),
           {:ok, publishes} <-
             ValueFlows.Planning.Intent.GraphQL.intent(
               %{id: publishes_intent_id},
               info
             ),
           {:ok, proposed_intent} <-
    ...
    def delete_proposed_intent(%{id: id}, info) do
      with :ok <- GraphQL.is_authenticated(info),
           ...
           {:ok, _} <-
             ValueFlows.Proposal.ProposedIntents.delete(proposed_intent) do
        {:ok, true}
      end
    end
  end
