...
defmodule ValueFlows.Proposal.Proposals do
  ...
  * Various parts of the codebase that need to query for this (inc. tests)
  """
  def many(filters \\ []),
    do: {:ok, repo().many(Queries.query(Proposal, filters))}
  ...
  * GraphQL resolver bulk resolution
  ...
  @spec create(any(), attrs :: map) ::
          {:ok, Proposal.t()} | {:error, Changeset.t()}
  def create(%{} = creator, attrs) when is_map(attrs) do
    attrs = prepare_attrs(attrs)

    repo().transact_with(fn ->
      with {:ok, proposal} <-
             repo().insert(Proposal.create_changeset(creator, attrs)),
           act_attrs = %{verb: "created", is_local: true},
           {:ok, activity} <-
             ValueFlows.Util.publish(creator, :propose, proposal) do
  ...
  @spec update(%Proposal{}, attrs :: map) ::
          {:ok, Proposal.t()} | {:error, Changeset.t()}
  def update(%Proposal{} = proposal, attrs) do
    ...
    repo().transact_with(fn ->
      with {:ok, proposal} <-
             repo().update(Proposal.update_changeset(proposal, attrs)),
  ...
  def ap_receive_activity(
        creator,
        activity,
        %{data: %{"publishes" => proposed_intents_attrs}} = object
      )
      when is_list(proposed_intents_attrs) and
             length(proposed_intents_attrs) > 0 do
    IO.inspect(object,
      label:
        "ap_receive_activity - handle Proposal with nested ProposedIntent (and usually Intent too)"
    )

    # remove nested objects to avoid double-creations
    proposal_attrs = object |> Utils.maybe_to_map() |> pop_in([:data, "publishes"]) |> elem(1)
    ...
    with {:ok, proposal} <-
      ...
      proposed_intents =
        for a_proposed_intent_attrs <- proposed_intents_attrs do
          a_proposed_intent_attrs = Map.put(a_proposed_intent_attrs, :published_in, proposal)
      ...
      {:ok, Map.put(proposal, :publishes, proposed_intents)}
    end
  end
