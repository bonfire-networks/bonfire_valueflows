...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.Process.GraphQL do
    ...
    def intended_inputs(process, %{action: action_id}, _)
    ...
    def intended_outputs(process, %{action: action_id}, _)
        when is_binary(action_id) do
    ...
    defp parse_search_params(search_params) do
      ...
      |> Enums.map_key_replace_existing(
        ...
        Util.maybe_classification_id(
          nil,
          Map.get(search_params, :classified_as)
        )
      )
      |> Keyword.new()
      |> IO.inspect()
    ...
    def delete_process(%{id: id}, info) do
      repo().transact_with(fn ->
        with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
             {:ok, process} <- process(%{id: id}, info),
             :ok <- ValueFlows.Util.can?(user, :delete, process),
