...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.Planning.Intent.GraphQL do
    ...
    def intents_filtered(%{filter: filters} = args, info)
        when is_map(filters) and filters != %{} do
      intents_filter(
        filters,
        [limit: Map.get(args, :limit, 10), offset: Map.get(args, :start, 0)],
        GraphQL.current_user(info)
      )
    end
    ...
    defp intents_filter(%{agent: id} = page_opts, filters_acc, current_user) do
      intents_filter_next(
        :agent,
        [agent_id: id_or_me(id, current_user)],
        page_opts,
        filters_acc,
        current_user
      )
    ...
    defp intents_filter(%{provider: id} = page_opts, filters_acc, current_user) do
      intents_filter_next(
        :provider,
        [provider_id: id_or_me(id, current_user)],
        page_opts,
        filters_acc,
        current_user
      )
    ...
    defp intents_filter(%{receiver: id} = page_opts, filters_acc, current_user) do
      intents_filter_next(
        :receiver,
        [receiver_id: id_or_me(id, current_user)],
        page_opts,
        filters_acc,
        current_user
      )
    ...
    defp intents_filter(
           %{in_scope_of: context_id} = page_opts,
           filters_acc,
           current_user
         ) do
      intents_filter_next(
    ...
    defp intents_filter(
           %{classified_as: tags} = page_opts,
           filters_acc,
           current_user
         ) do
      intents_filter_next(
        :classified_as,
        [tag_ids: Util.maybe_classification_id(current_user, tags)],
        page_opts,
        filters_acc,
        current_user
      )
    ...
    defp intents_filter(
           %{at_location: at_location_id} = page_opts,
           filters_acc,
           current_user
         ) do
      intents_filter_next(
            ...
            %{
              geolocation:
                Map.merge(geolocation, %{
                  ...
                  distance:
                    Map.get(geolocation, :distance, %{
                      meters: @radius_default_distance
                    })
                })
            }
          ),
    ...
    defp intents_filter_next(
      ...
      intents_filter(
        Map.drop(page_opts, param_remove),
        filters_acc ++ filter_add,
        current_user
      )
    end
    ...
    defp id_or_me(["me"], current_user),
      do: Types.uid(current_user) || raise(Bonfire.Fail.Auth, :needs_login)
    ...
    def fetch_resource_inventoried_as_edge(
          %{resource_inventoried_as_id: id} = thing,
          _,
          _
        )
