if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.Planning.Commitment.GraphQL do
    ...
    alias ValueFlows.Planning.Commitment.Commitments
    ...
    def commitments_filtered(%{filter: filts} = args, _info)
        when is_map(filts) do
    ...
    def fetch_resource_inventoried_as_edge(
          %{resource_inventoried_as_id: id} = comm,
          _,
          _
        )
    ...
    def fetch_input_of_edge(%{input_of_id: id} = comm, _, _)
        when is_binary(id) do
    ...
    def fetch_output_of_edge(%{output_of_id: id} = comm, _, _)
        when is_binary(id) do
